...
def generate_summary(state: State) -> dict:
    prompt = f"""
Ενσαρκώνεις τον ρόλο ενός έμπειρου επαγγελματία δημοσιογράφου. Σκοπός σου είναι να συντάξεις συνοπτική, πλήρη και ακριβή εξαγωγή των κύριων ιδεών/σημείων του παρακάτω ελληνικού άρθρου, σε μορφή bullets.
 
Οδηγίες:
- Δημιούργησε 3-4 bullets.
- Καθε bullet να έχει μέγιστο αριθμό λέξεων 45.
...
- Τα bullets πρέπει να βασίζονται αυστηρά στο περιεχόμενο του άρθρου, χωρίς προσθήκες ή εικασίες.
-Σημείωση: Ορισμένες λέξεις ή φράσεις εμφανίζονται ανάμεσα σε διπλούς αστερίσκους (π.χ. **παράδειγμα**). Αυτό σημαίνει ότι στο αρχικό άρθρο ήταν γραμμένες με έντονη γραφή (bold) και μπορεί να είναι σημαντικές για την εξαγωγή των bullets.:
